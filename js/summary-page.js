...
(() => {
  ...
  const K = { last: "orbita:summary:last", history: "orbita:summary:history", excluded: "orbita:summary:excluded", resolved: "orbita:summary:resolved" };
  ...
  const S = { period: "last", running: false, cancel: false, progress: null, current: null, history: [], excluded: [], resolved: {}, last: 0, groups: null, showExcl: false, showResolved: false };
  ...
  function resultHtml(s) {
    ...
    return `<div class="card sec head">
        <div class="row"><div style="flex:1"><h2 style="font-size:17px">Resumo de ${esc(when(s.at))}</h2><small class="muted">${esc(PERIODS[s.period] || "")} · desde ${esc(when(s.since))} · ${nf.format(t.chats)} conversas · ${nf.format(t.messages)} mensagens${s.partial ? " · <b>interrompido</b>" : ""}</small></div>
          <label class="row small" style="gap:6px"><input type="checkbox" data-a="showres" ${S.showResolved ? "checked" : ""}> Mostrar resolvidos</label></div>
  ...
  function render() {
    ...
    app.innerHTML = `<div class="top"><div class="grow"><h1>Resumo do WhatsApp</h1><p>Perguntas para você, compromissos, avisos dos grupos — com a fonte de cada item.</p></div></div>
      <div class="card sec">
        ...
        <p class="muted small" style="margin:0">As mensagens do período vão para o provedor de IA das Opções para serem resumidas. O WhatsApp Web precisa estar aberto numa aba.</p>
      </div>
      ${exclHtml()}
  ...
  (async () => {
    [S.last, S.history, S.excluded, S.resolved] = await Promise.all([get(K.last, 0), get(K.history, []), get(K.excluded, []), get(K.resolved, {})]);
